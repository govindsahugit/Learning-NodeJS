...
router.get("/{:id}", async (req, res) => {
  ...
  const directoryData = id
    ...
    : directoriesData.find((folder) => folder.id === req.user.rootDirId);
...
router.delete("/:id", async (req, res, next) => {
  ...
  const deleteDir = async (id) => {
    ...
    dirData?.files?.forEach(async (fileId) => {
      ...
      if (!file)
        return res.status(404).send({ message: "Director file not found!" });
      const indexOfFile = filesData.indexOf(file);
      if (indexOfFile === -1)
        return res.status(404).json({ message: "Directory file not found!" });
